...
function CategoryPills() {
    ...
    return (
        <div className="px-4 pt-6 pb-2 bg-[var(--c-bg)]">
            <div
                ref={scrollRef}
            ...
            >
                <button
                    ...
                    className={`shrink-0 px-6 py-2.5 rounded-full text-[13px] font-bold tracking-tight transition-all duration-300 ${
                        active === "all"
                            ? "bg-[var(--c-primary)] text-white shadow-lg shadow-[var(--c-primary)]/20 border border-[var(--c-primary)]"
                            : "bg-[var(--c-surface-alt)]/50 text-[var(--c-text)] border border-transparent hover:border-[var(--c-border)]"
                    }`}
                >
                    All Products
                </button>
            ...
            <div className="h-[2px] w-full bg-[var(--c-primary)]/10 rounded-full mb-6">
                <div className="h-full w-24 bg-[var(--c-primary)] rounded-full" />
            </div>
        </div>
    );
}
        ...
        <section className="py-2 px-4 mb-8">
            <div className="flex items-end justify-between mb-6">
                <div className="flex items-center gap-2">
                    {Icon && <Icon className="w-5 h-5 text-[var(--c-accent)]" />}
                    <h2 className="text-[26px] font-serif font-bold text-[var(--c-text)] tracking-tight">{title}</h2>
                </div>
                <Link href={viewAllLink || "/products"} className="flex items-center gap-1 text-[14px] text-[var(--c-text-muted)] font-medium hover:text-[var(--c-primary)] transition-colors">
                    See All
...
function SimpleProductCard({ product }) {
    ...
    return (
        <Link href={`/products/${product._id}`} className="flex flex-col group h-full">
            <div className="aspect-square rounded-[2rem] overflow-hidden bg-white border border-[var(--c-border)] shadow-sm relative group">
                ...
                {product.isFeatured && (
                    <div className="absolute top-3 left-3 bg-[var(--c-accent)] text-white text-[9px] font-extrabold px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-sm">
                         <Flame className="w-2.5 h-2.5 fill-white" /> HOT
                    </div>
                )}
                {hasSale && (
                    <div className="absolute top-3 right-3 bg-[var(--c-star)] text-[var(--c-text)] text-[9px] font-extrabold px-2.5 py-1 rounded-lg shadow-sm">
                        {discountPercent}% OFF
                    </div>
                )}
                {product.isActive && !product.isFeatured && (
                    <div className="absolute top-3 left-3 bg-[var(--c-primary)] text-white text-[9px] font-extrabold px-2.5 py-1 rounded-lg shadow-sm">
                        NEW
                    </div>
                )}
            </div>
            <div className="px-1 pt-4 flex flex-col flex-1">
                <h3 className="font-serif text-[16px] font-bold text-[var(--c-text)] line-clamp-1 leading-tight">{product.name}</h3>
                <p className="text-[11px] text-[var(--c-text-muted)] mt-1 line-clamp-1 italic">
                    {product.shortDescription || "Handcrafted with artisanal spices"}
                </p>
                <div className="flex items-center justify-between mt-auto pt-3">
                    <div className="flex items-baseline gap-1.5">
                        <span className="text-[16px] font-bold text-[var(--c-text)]">₹{price}</span>
                        {hasSale && <span className="text-[11px] text-[var(--c-text-muted)] line-through">₹{originalPrice}</span>}
                    </div>
                    <div className="flex items-center gap-0.5 bg-[var(--c-accent-soft)] px-2 py-0.5 rounded-full">
                        <Star className="w-2.5 h-2.5 fill-[var(--c-accent)] text-[var(--c-accent)]" />
                        <span className="text-[10px] font-bold text-[var(--c-text)]">{product.avgRating || '4.8'}</span>
                    </div>
                </div>
            </div>
        </Link>
...
function HorizontalProductCard({ product }) {
    ...
    const router = useRouter();
    return (
        <Link href={`/products/${product._id}`} className="flex gap-4 bg-white rounded-[2rem] p-3 shadow-sm border border-[var(--c-border)] mb-4 group transition-all hover:shadow-md">
            <div className="w-24 h-24 shrink-0 rounded-2xl overflow-hidden bg-[var(--c-bg)] border border-[var(--c-border)]/50">
            ...
            <div className="flex flex-col flex-1 py-1">
                <span className="inline-block self-start bg-[var(--c-accent-soft)] text-[var(--c-primary)] text-[9px] font-bold px-2 py-0.5 rounded-full mb-1 tracking-widest uppercase">New Arrival</span>
                <h3 className="font-serif text-[16px] font-bold text-[var(--c-text)] line-clamp-1 leading-tight">{product.name}</h3>
                <p className="text-[11px] text-[var(--c-text-muted)] line-clamp-1 mt-1">{product.shortDescription || "Premium quality handcrafted product"}</p>
                <div className="flex items-center justify-between mt-auto">
                    <span className="text-[16px] font-bold text-[var(--c-text)]">₹{price}</span>
                    <div className="flex items-center gap-0.5">
                        <Star className="w-3 h-3 fill-[var(--c-accent)] text-[var(--c-accent)]" />
                        <span className="text-[11px] font-bold text-[var(--c-text)]">{product.avgRating || '4.9'}</span>
                    </div>
                </div>
            </div>
        </Link>
...
function MobilePromoBanner() {
    return (
        <div className="px-4 py-6 mb-4">
            <div className="relative w-full h-[160px] rounded-[2.5rem] overflow-hidden bg-[var(--c-text)] shadow-xl group">
                {/* Image overlay */}
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-[var(--c-text)] via-[var(--c-text)]/60 to-transparent p-6 flex flex-col justify-center">
                    <div className="inline-flex items-center gap-1.5 text-[var(--c-accent)] text-[10px] font-bold uppercase tracking-[0.2em] mb-2">
...
function SpecialOffers() {
    const offers = [
        { amount: "10", type: "Discount", title: "Welcome Gift", color: "from-[var(--c-primary)] to-[#606E4D]" },
        { amount: "20", type: "Discount", title: "Summer Sale", color: "from-[var(--c-accent)] to-[#B35236]" },
        { amount: "30", type: "Discount", title: "Bulk Joy", color: "from-[var(--c-text)] to-[#1A1F18]" }
    ];
    return (
        <div className="px-4 py-6 mb-8 bg-[#FCF9F2] border-y border-[var(--c-border)]/50 overflow-hidden">
            <div className="inline-flex items-center gap-2 mb-4">
                <div className="h-px w-6 bg-[var(--c-accent)]/30" />
                <h2 className="text-[10px] font-bold text-[var(--c-accent)] uppercase tracking-[0.3em]">Exclusive Offers</h2>
                <div className="h-px w-6 bg-[var(--c-accent)]/30" />
            </div>
                    ...
                    <div key={idx} className="shrink-0 w-[200px] snap-start">
                        <div className={`relative h-[170px] rounded-[2.5rem] overflow-hidden bg-gradient-to-br ${off.color} p-6 shadow-xl shadow-black/10 flex flex-col justify-between group`}>
                            ...
                            <div className="relative z-10 flex items-end justify-between">
                                ...
                                <button className="bg-white text-[var(--c-text)] text-[11px] font-bold px-5 py-2.5 rounded-full shadow-lg transform active:scale-95 transition-all">
                                    Claim Now
                                </button>
                            </div>
                        </div>
...
export default function MobileProductSections() {
    ...
    if (loading) {
        return <div className="lg:hidden h-screen bg-[var(--c-bg)] flex items-center justify-center"><div className="w-8 h-8 border-3 border-[var(--c-primary)] border-t-transparent flex rounded-full animate-spin"></div></div>;
    }

    return (
        <div className="lg:hidden bg-[var(--c-bg)] min-h-screen pb-16">
